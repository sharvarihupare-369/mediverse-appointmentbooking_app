...
const Doctors = () => {
  ...
  const fetchDoctorData = (page,orderBy,orderByRating,filterBy) => {
    let api = getapis(`http://localhost:8080/doctors?_page=${page}&_limit=12`,orderBy,orderByRating,filterBy)
    setLoading(true);
    axios
      .get(api)
      .then(res => {
  ...
  return (
   ...
   <VStack ml="50px">
    ...
    <Box>
    <FormLabel>
          Filter by Category:</FormLabel>
          <Select className="filter-by-category" value={filterBy} onChange={(e)=>setFilterBy(e.target.value)} >
        ...
        {data.length == 0 ? (
          ...
          data.map((doctor, ind) => {
            ...
            return (
              ...
              >
                <Flex justifyContent={"flex-end"} gap="3px" alignItems={"center"}>
